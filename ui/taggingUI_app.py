...
import ui.meta_windows as meta
import nestor
...
Ui_MainWindow_taggingTool, QtBaseClass_taggingTool = uic.loadUiType(
    script_dir/fname)
...
class MyTaggingToolWindow(Qw.QMainWindow, Ui_MainWindow_taggingTool):
    ...
    def __init__(
        ...
        Instantiate  values
        """
        self.helpTutorialLink = (
            "https://nestor.readthedocs.io/en/latest/"
            "how_to_guide/tutorial.html"
        )
        ...
        self.existingProject = set(
            [folder.name for folder in projectsPath.iterdir()
                if folder.is_dir()])
        ...
        self.config_default = {
            ...
            'database': {
                'schema': str(
                    script_dir.parent / 'store_data' / 'DatabaseSchema.yaml'
                )
            },
            'classification': {
                'mapping': {
        ...
        self.enableFeature(existDatabase=False,
                           existProject=False, existTagExtracted=False)
        ...
        self.completenessPlot = MyMplCanvas(
            self.gridLayout_report_progressPlot,
            self.tabWidget,
            self.dataframe_completeness,
            )
        self.horizontalSlider_1gram_FindingThreshold.setValue(
            self.config['settings'].get('showCkeckBox_threshold', 50))
        ...
        self.buttonGroup_similarityPattern = QButtonGroup_similarityPattern(
            self.verticalLayout_1gram_SimilarityPattern)
        ...
        self.actionReplace_special_words.triggered.connect(
            self.setMenu_specialReplace)
        ...
        self.actionOpen_Database.triggered.connect(
            self.setMenu_databaseOpenBrowser)
        self.actionDisconnect.triggered.connect(
            self.setMenu_databaseDisconnect)

        self.action_AutoPopulate_FromDatabase_1gramVocab.triggered.connect(
            self.setMenu_autopopulateFromDatabase_1gVocab)
        self.action_AutoPopulate_FromDatabase_NgramVocab.triggered.connect(
            self.setMenu_autopopulateFromDatabase_NgVocab)
        self.action_AutoPopulate_FromCSV_1gramVocab.triggered.connect(
            self.setMenu_autopopulateFromCSV_1gVocab)
        self.action_AutoPopulate_FromCSV_NgramVocab.triggered.connect(
            self.setMenu_autopopulateFromCSV_NgVocab)
        self.actionFrom_AutoPopulate_From1gramVocab.triggered.connect(
            self.setMenu_autopopulateNgramFrom1gram)
        ...
        self.tableWidget_1gram_TagContainer.itemSelectionChanged.connect(
            self.onSelect_tableViewItems1gramVocab)
        self.tableWidget_Ngram_TagContainer.itemSelectionChanged.connect(
            self.onSelect_tableViewItemsNgramVocab)

        self.buttonGroup_NGram_Classification.buttonClicked.connect(
            self.setAliasFromNgramButton)
        self.horizontalSlider_1gram_FindingThreshold.sliderMoved.connect(
            self.onMoveSlider_similarPattern)
        self.horizontalSlider_1gram_FindingThreshold.sliderReleased.connect(
            self.onMoveSlider_similarPattern)
        self.pushButton_1gram_UpdateTokenProperty.clicked.connect(
            self.onClick_Update1GramVocab)
        self.pushButton_Ngram_UpdateTokenProperty.clicked.connect(
            self.onClick_UpdateNGramVocab)

        self.pushButton_report_saveTrack.clicked.connect(
            self.onClick_saveTrack)
        self.pushButton_report_saveNewCsv.clicked.connect(
            self.onClick_saveNewCsv)
        self.pushButton_report_saveH5.clicked.connect(
            self.onClick_saveTagsHDFS)

        self.dialogTOU = meta.DialogMenu_TermsOfUse()
        ...
        Set the shortcut
        """
        Qw.QShortcut(Qg.QKeySequence("Ctrl+N"),
                     self).activated.connect(self.setMenu_projectNew)
        self.actionNew_Project.setText(
            self.actionNew_Project.text() + "\tCtrl+N")
        Qw.QShortcut(Qg.QKeySequence("Ctrl+S"),
                     self).activated.connect(self.setMenu_projectSave)
        self.actionSave_Project.setText(
            self.actionSave_Project.text() + "\tCtrl+S")
        Qw.QShortcut(Qg.QKeySequence("Ctrl+O"),
                     self).activated.connect(self.setMenu_projectOpen)
        self.actionOpen_Project.setText(
            self.actionOpen_Project.text() + "\tCtrl+O")
        Qw.QShortcut(Qg.QKeySequence("Ctrl+D"),
                     self).activated.connect(self.setMenu_databaseConnect)
        self.actionConnect.setText(self.actionConnect.text() + "\tCtrl+D")

        Qw.QShortcut(Qg.QKeySequence("Alt+1"), self).activated.connect(
                lambda: self.tabWidget.setCurrentIndex(0)
        )
        Qw.QShortcut(Qg.QKeySequence("Alt+2"), self).activated.connect(
            lambda: self.tabWidget.setCurrentIndex(1)
        )
        Qw.QShortcut(Qg.QKeySequence("Alt+3"), self).activated.connect(
        ...
        Research Mode
        """
        self.actionResearch_NewProject.triggered.connect(
            self.setMenu_researchProject_new)
    ...
    def center(self):
        ...
        screen = Qw.QApplication.desktop().screenNumber(
            Qw.QApplication.desktop().cursor().pos())
    ...
    def setMenu_researchProject_new(self):
        ...
        :return:
        """
        dialogMenu_newResearchProject = meta.DialogMenu_ResearchWindow(
            self.iconPath
        )
        ...
        (dialogMenu_newResearchProject
         .comboBox_ResearchWindows_projectName
         .addItems(["excavator_data"])
         )

        (dialogMenu_newResearchProject
         .lineEdit_ResearchWindows_saveVocab_percentage
         .setText("0,5,10,20,30,40,50,60")
         )
        (dialogMenu_newResearchProject
         .lineEdit_ResearchWindows_saveVocab_time
         .setText("0,1,2,3,4,5,10,20,30,40,50,60")
         )
        (dialogMenu_newResearchProject
         .lineEdit_ResearchWindows_saveVocab_nbtoken
         .setText("0,10,20,50,100,200,300,400,500,1000")
         )
        (dialogMenu_newResearchProject
         .lineEdit_ResearchWindows_saveVocab_nbUpdate
         .setText("0,10,20,50,100,200,300,400,500,1000")
         )
        ...
        def onclick_ok():
            (saveType,
             ...
             saveNumberToken,
             saveNumberUpdate
             ) = dialogMenu_newResearchProject.get_data()

            if saveType and author:
                (dialogMenu_newResearchProject
                 .label_ResearchWindows_saveVocab
                 .setStyleSheet(self.changeColor['default'])
                 )
                (dialogMenu_newResearchProject
                 .lineEdit_ResearchWindows_projectAuthor
                 .setStyleSheet(self.changeColor['default'])
                 )
                ...
                self.__init__(
                    ...
                    saveNumberToken=saveNumberToken,
                    saveNumberUpdate=saveNumberUpdate,
                )
                ...
                (dialogMenu_newResearchProject
                 .label_ResearchWindows_saveVocab
                 .setStyleSheet(self.changeColor['wrongInput'])
                 )
                (dialogMenu_newResearchProject
                 .lineEdit_ResearchWindows_projectAuthor
                 .setStyleSheet(self.changeColor['wrongInput'])
    ...
    def setMenu_projectNew(self):
        ...
        dialogMenu_newProject = meta.DialogMenu_newProject(self.iconPath)
        self.setEnabled(False)
        ...
        def onclick_ok():
            ...
            (name,
             ...
             vocabNg,
             pathCSV_old,
             ) = dialogMenu_newProject.get_data()

            if name and pathCSV_old:
                (dialogMenu_newProject
                 .lineEdit_NewProject_ProjectName
                 .setStyleSheet(self.changeColor['default'])
                 )
                (dialogMenu_newProject
                 .lineEdit_NewProject_LoadCSV
                 .setStyleSheet(self.changeColor['default'])
                 )
                if name not in self.existingProject:
                    ...
                    # open the dataframe, and
                    # save is as utf8 on the project localisation
                    dataframe_tmp = openDataframe(pathCSV_old)
                    dataframe_tmp.to_csv(
                        pathCSV_new/self.config['original'],
                        encoding='utf-8-sig'
                    )
                    ...
                    Qw.QMessageBox.about(
                        self, 'Name alreay exists',
                        ("The name you have writen already exists, "
        ...
        dialogMenu_newProject.buttonBox__NewProject.accepted.connect(
            onclick_ok)
    ...
    def setMenu_projectOpen(self):
        ...
        dialogMenu_openProject = meta.DialogMenu_openProject(
            self.iconPath,
            self.projectsPath,
        ...
        def onclick_ok():
            if (dialogMenu_openProject
                    .comboBox_OpenProject_ProjectName
                    .currentText()):
        ...
        def onclick_remove():
            if (dialogMenu_openProject
                    .comboBox_OpenProject_ProjectName
                    .currentText()) != self.config.get("name", "None"):
                if (dialogMenu_openProject
                        .comboBox_OpenProject_ProjectName
                        .currentText()) != "":
                    ...
                    if choice == Qw.QMessageBox.Yes:
                        ...
                        remove_folderContent(
                            self.projectsPath
                            / (dialogMenu_openProject
                                .comboBox_OpenProject_ProjectName
                                .currentText())
                        )
                        self.existingProject.remove(
                            dialogMenu_openProject
                            .comboBox_OpenProject_ProjectName
                            .currentText()
                        )
                    ...
                    print("NOT ALLOW --> "
                          "You cannot delete your current project")

        (dialogMenu_openProject
         .pushButton_OpenProject_ProjectRemove
         .clicked
         .connect(onclick_remove)
         )
        (dialogMenu_openProject
         .buttonBox_OpenProject
         .accepted
         .connect(onclick_ok)
         )
    ...
    def setMenu_projectSave(self):
        ...
        if projectName:
            ...
            if self.dataframe_vocab1Gram is not None:
                ...
                self.dataframe_vocab1Gram.to_csv(
                    vocab1gPath, encoding='utf-8-sig')

                vocabNgPath = self.config.get('vocabNg', 'vocabNg') + ".csv"
                vocabNgPath = folderPath / vocabNgPath
                self.dataframe_vocabNGram.to_csv(
                    vocabNgPath, encoding='utf-8-sig')
    ...
    def setMenu_databaseConnect(self):
        ...
        :return:
        """
        dialogMenu_databaseConnect = meta.DialogMenu_DatabaseConnect(
        ...
        def onclick_ok():
            (username,
             ...
             browserport,
             password
             ) = dialogMenu_databaseConnect.get_data()
            ...
            try:
                ...
                (dialogMenu_databaseConnect
                 .lineEdit_DialogDatabaseConnection_Username
                 .setStyleSheet(self.changeColor['wrongInput'])
                 )
                (dialogMenu_databaseConnect
                 .lineEdit_DialogDatabaseConnection_Password
                 .setStyleSheet(self.changeColor['wrongInput'])
                 )
            except (neo4j.exceptions.AddressError,
                    neo4j.exceptions.ServiceUnavailable):
                (dialogMenu_databaseConnect
                 .lineEdit_DialogDatabaseConnection_ServerPortNumber
                 .setStyleSheet(self.changeColor['wrongInput'])
                 )
                (dialogMenu_databaseConnect
                 .lineEdit_DialogDatabaseConnection_ServerName
                 .setStyleSheet(self.changeColor['wrongInput'])
                 )
                (dialogMenu_databaseConnect
                 .lineEdit_DialogDatabaseConnection_Username
                ...
                (dialogMenu_databaseConnect
                 .lineEdit_DialogDatabaseConnection_OpenSchema
                 .setStyleSheet(self.changeColor['wrongInput'])
                 )
                (dialogMenu_databaseConnect
                 .lineEdit_DialogDatabaseConnection_ServerPortNumber
        ...
        (dialogMenu_databaseConnect
         .buttonBox_DialogDatabaseConnection
         .accepted.connect(onclick_ok)
         )
    ...
    def setMenu_databaseRunQuery(self):
        ...
        dialogMenu_databaseRunQuery = meta.DialogMenu_DatabaseRunQueries(
            iconPath=self.iconPath,
        ...
        (dialogMenu_databaseRunQuery
         .button_DialogDatabaseRunQuery
         .accepted.connect(onclick_ok)
         )
    ...
    def setMenu_autopopulateFromDatabase_1gVocab(self):
        ...
        if done:
            ...
            mask = self.dataframe_vocab1Gram[[
                "NE", "alias"]].isnull().all(axis=1)

            df_tmp = self.dataframe_vocab1Gram.loc[mask, :]
            df_tmp.update(other=df, overwrite=False)
            df_tmp = df_tmp.dropna(thresh=3)

            # get the list of updated index localisation
            # from dataframe_vocab1Gram
            dataframe_vocab1Gram_copy = self.dataframe_vocab1Gram.reset_index()
            self.tokenUpdate1g_database.update(
                dataframe_vocab1Gram_copy.
                index[
                    dataframe_vocab1Gram_copy['tokens']
            ...
            self.printDataframe_TableviewProgressBar(
                dataframe=self.dataframe_vocab1Gram,
    ...
    def setMenu_autopopulateFromDatabase_NgVocab(self):
        ...
        if done:
            ...
            mask = (
             self.dataframe_vocabNGram[["NE", "alias"]]
             .isnull()
             .all(axis=1)
            )
            ...
            self.printDataframe_TableviewProgressBar(
                dataframe=self.dataframe_vocabNGram,
    ...
    def setMenu_autopopulateFromCSV_1gVocab(self):
        ...
        fileName, _ = Qw.QFileDialog.getOpenFileName(
            self,
            self.objectName(),
            "Open NESTOR generated vocab File",
            "csv Files (*.csv)",
            options=options,
        )
        ...
        if fileName:
            ...
            df = pd.read_csv(fileName)[
                             ["tokens", "NE", "alias"]].set_index("tokens")
            ...
            # from dataframe_vocab1Gram
            dataframe_vocab1Gram_copy = self.dataframe_vocab1Gram.reset_index()
            self.tokenUpdate1g_vocab.update(
                dataframe_vocab1Gram_copy.
                index[
                    dataframe_vocab1Gram_copy['tokens']
                    .isin(df_tmp.index.tolist())
    ...
    def setMenu_autopopulateFromCSV_NgVocab(self):
        ...
        if fileName:
            ...
            mask = self.dataframe_vocabNGram[[
                "NE", "alias"]].isnull().all(axis=1)
            ...
            # from dataframe_vocab1Gram
            dataframe_vocabNGram_copy = self.dataframe_vocabNGram.reset_index()
            self.tokenUpdateNg_vocab.update(
                dataframe_vocabNGram_copy.
                index[
                    dataframe_vocabNGram_copy['tokens']
                    .isin(df_tmp.index.tolist())
                ].tolist()
            )
    ...
    def setMenu_settings(self):
        ...
        dialogMenu_settings = meta.DialogMenu_settings(
        ...
        def onclick_ok():
            ...
            (name,
             ...
             vocabNg,
             numberTokens,
             alreadyChecked_threshold,
             showCkeckBox_threshold,
             ) = dialogMenu_settings.get_data()

            if name and name not in self.existingProject:

                oldnumberToken = self.config["settings"].get(
                    "numberTokens", 1000)

                Path(str(self.projectsPath / self.config.get("name")
                         / self.config.get("vocab1g")) + ".csv").unlink()
                Path(str(self.projectsPath / self.config.get("name")
                         / self.config.get("vocabNg")) + ".csv").unlink()

                (dialogMenu_settings
                 .lineEdit_Settings_ProjectName
                 .setStyleSheet(self.changeColor['default'])
                 )
                ...
                if oldnumberToken != (
                    self.config["settings"].get("numberTokens", 1000)
                ) or (
                    self.config["csvinfo"].get("untracked_token", None)
                ):
    ...
    def setMenu_mapCsvHeader(self):
        ...
        self.dialogMenu_csvHeaderMapping = meta.DialogMenu_csvHeaderMapping(
            csvHeaderContent=list(self.dataframe_Original.columns.values),
        ...
        (self.dialogMenu_csvHeaderMapping
         .buttonBox_csvHeaderMapping.accepted
         .connect(onclick_ok)
         )

    def setMenu_specialReplace(self):
        dialogMenu_specialReplace = meta.DialogMenu_SpecialReplace(
            iconPath=self.iconPath,
        ...
        def onclick_ok():
            self.set_config(
                untrackedTokenList=dialogMenu_specialReplace.get_data())

            self.whenProjectOpen()

            dialogMenu_specialReplace.close()

        (dialogMenu_specialReplace
         .buttonBox_specialReplace
         .accepted.connect(onclick_ok)
         )
    ...
    def setMenu_ExportZip(self, format):
        ...
        :return:
        """
        target = str(Qw.QFileDialog.getExistingDirectory(
    ...
    def setMenu_Import(self):
        ...
        if fileName:
            ...
            if fileName.stem not in self.existingProject:
                ...
                shutil.unpack_archive(str(fileName), str(
                    self.projectsPath/fileName.stem))
                self.config = openYAMLConfig_File(
                    self.projectsPath/fileName.stem/'config.yaml')

                self.whenProjectOpen()
            else:
                print("ISSUE --> the project ", fileName.stem,
                      " already exists localy, so it cannot be imported")

    def printDataframe_TableviewProgressBar(
        self, dataframe, tableview, progressBar
    ):
        """
        ...
        if dataframe is not None:
            ...
            try:
                for index in tableview.userUpdate:
                    if index < 1000:
                        tableview.item(index, 0).setBackground(
            ...
            tableview.setHorizontalHeaderLabels(
                temp_df.columns.tolist()[:-1])  # ignore score column
            tableview.setSelectionBehavior(Qw.QTableWidget.SelectRows)

            header = tableview.horizontalHeader()
            # notes column can stretch
    ...
    def changeTableview_color(self):
        ...
        for t in self.tokenUpdate1g_database:
            if t < max_token_num:
                self.tableWidget_1gram_TagContainer.item(t, 0).setBackground(
                    self.changeColor['updateToken_database'])

        for t in self.tokenUpdateNg_database:
            if t < max_token_num:
                self.tableWidget_Ngram_TagContainer.item(t, 0).setBackground(
                    self.changeColor['updateToken_database'])

        for t in self.tokenUpdate1g_vocab:
            if t < max_token_num:
                self.tableWidget_1gram_TagContainer.item(t, 0).setBackground(
                    self.changeColor['updateToken_vocab'])
        ...
        for t in self.tokenUpdateNg_user:
            if t < max_token_num:
                self.tableWidget_Ngram_TagContainer.item(
                    t, 0).setBackground(self.changeColor['updateToken_user'])
    ...
    def onSelect_tableViewItems1gramVocab(self):
        ...
        """
        # selected row
        items = self.tableWidget_1gram_TagContainer.selectedItems()
        ...
        self.classificationDictionary_1Gram.get(
            classification,
            self.radioButton_1gram_NotClassifiedEditor
        ).setChecked(True)
        ...
        self.buttonGroup_similarityPattern.create_checkBoxs(
            token=token,
            autoCheck_value=self.config['settings'].get(
    ...
    def onMoveSlider_similarPattern(self):
        ...
        try:
            self.buttonGroup_similarityPattern.create_checkBoxs(
                token=(self.tableWidget_1gram_TagContainer
                           .selectedItems()[0]
                           .text()),
                autoCheck_value=(self.config['settings']
                                     .get('alreadyChecked_threshold', 50)),
                checkBox_show=(self.horizontalSlider_1gram_FindingThreshold
                                   .value()),
            )
        except IndexError:
    ...
    def onSelect_tableViewItemsNgramVocab(self):
        ...
        self.label_Ngram_CompositionDescription.setToolTip(
            '<font>'+tooltip+'</font>')
        ...
        self.classificationDictionary_NGram.get(
            classification,
            self.radioButton_Ngram_NotClassifiedEditor
        ).setChecked(True)
        ...
        for tok1g in token.split(" "):
            ...
            try:
                firstToken = (
                    self.dataframe_vocab1Gram
                        .loc[self.dataframe_vocab1Gram['alias'] == tok1g]
                        .iloc[0]
                )
            except IndexError:
                firstToken = pd.Series({
                    "NE": "",
                    "alias": "",
                    "note": "",
                    "score": "",
            ...
            similarityList = "\n".join(
                self.dataframe_vocab1Gram
                    .index[self.dataframe_vocab1Gram['alias'] == tok1g]
                    .tolist()
            )

            labelValueSimilarity = Qw.QLabel()
            labelValueSimilarity.setText(similarityList)
            labelValueSimilarity.setObjectName(
                "labelNGramComposition" + 'aliasValue')
            self.gridLayout_Ngram_NGramComposedof.addWidget(
                labelValueSimilarity, xPos, yValue)
            ...
            self.gridLayout_Ngram_NGramComposedof.addWidget(
                separator, xPos, yLabel)

            xPos += 1

        verticalSpacer = Qw.QSpacerItem(
            20, 40, Qw.QSizePolicy.Minimum, Qw.QSizePolicy.Expanding)
    ...
    def onClick_Update1GramVocab(self):
        ...
        try:
            # selected row
            items = self.tableWidget_1gram_TagContainer.selectedItems()
            token, classification, alias, notes = (
                str(i.text()) for i in items)

            for btn in self.buttonGroup_similarityPattern.buttons():
                if btn.isChecked():
                    new_alias = self.buttonDictionary_1Gram.get(
                        (self.buttonGroup_1Gram_Classification
                             .checkedButton()
                             .text()
                         ), '')

                    self.tokenUpdate1g_user.add(
                        self.dataframe_vocab1Gram.index.get_loc(btn.text()))

                    self.dataframe_vocab1Gram.at[btn.text(
                    ), 'alias'] = self.lineEdit_1gram_AliasEditor.text()
                    self.dataframe_vocab1Gram.at[btn.text(
                    ), 'notes'] = self.textEdit_1gram_NoteEditor.toPlainText()
                    self.dataframe_vocab1Gram.at[btn.text(), 'NE'] = new_alias

            # remove the information for the token that WAS the same but
            # the user did not want it to be the same anymore
            # only if they had the same alias
            for textToUncheck in (self.buttonGroup_similarityPattern
                                      .textToUncheck):
                if (self.dataframe_vocab1Gram
                        .at[textToUncheck, "alias"]) == alias:
                    ...
                    self.tokenUpdate1g_user.add(
                        self.dataframe_vocab1Gram.index.get_loc(textToUncheck))
            ...
            self.tableWidget_1gram_TagContainer.selectRow(
                self.tableWidget_1gram_TagContainer.currentRow() + 1)
            self.lineEdit_1gram_AliasEditor.setFocus()
        except (IndexError, ValueError):
            Qw.QMessageBox.about(self, 'Can\'t select',
    ...
    def onClick_UpdateNGramVocab(self):
        ...
        try:
            ...
            self.tokenUpdateNg_user.add(
                self.dataframe_vocabNGram.index.get_loc(token)
            )

            (self.dataframe_vocabNGram.at[token, 'alias']) = (
                    self.lineEdit_Ngram_AliasEditor.text()
                 )
            (self.dataframe_vocabNGram.at[token, 'notes']) = (
                self.textEdit_Ngram_NoteEditor.toPlainText()
             )
            self.dataframe_vocabNGram.at[token, 'NE'] = (
                self.buttonDictionary_NGram.get(
                    self.buttonGroup_NGram_Classification
                        .checkedButton()
                        .text(),
                    '')
            )

            if self.buttonDictionary_NGram.get(
            ...
            self.tableWidget_Ngram_TagContainer.selectRow(
                self.tableWidget_Ngram_TagContainer.currentRow() + 1)
            self.lineEdit_Ngram_AliasEditor.setFocus()
        except (IndexError, ValueError):
            Qw.QMessageBox.about(self, 'Can\'t select',
                                 "You should select a row first")

    # name=None, author=None, description=None,
    # vocab1g=None, vocabNg=None, original=None,
    ...
    def set_config(self, **kwargs):
        ...
        in the config file. It Reload all the printing and stuff
        :param name:
        :param author:
        :param description:
        :param vocab1g:
        ...
        #     self.config['settings']["alreadyChecked_threshold"]
        # = alreadyChecked_threshold
        # if showCkeckBox_threshold is not None:
        #     self.config['settings']["showCkeckBox_threshold"]
        # = showCkeckBox_threshold
        #
    ...
    def whenProjectOpen(self):
        """
        This function will execute all the changes when you create / open /
        import a new project

        self.config needs to be updated with the new project
        :return:
        """
        self.set_config(schema=str(script_dir.parent
                                   / 'store_data' / 'DatabaseSchema.yaml'))
        ...
        # Open dataframs
        self.dataframe_Original = openDataframe(
            self.projectsPath / self.config['name'] / self.config['original'])
        ...
        # if we open a project, init the new dataframe
        # with the one in the vocab
        if vocab1gPath.exists():
            self.extract_1gVocab(vocab1gPath, openDataframe(
                vocab1gPath).fillna("").set_index("tokens"))
        # if new project, just create the dataframe
        else:
            self.extract_1gVocab(vocab1gPath)

        if vocabNgPath.exists():
            self.extract_NgVocab(init=openDataframe(
                vocabNgPath).fillna("").set_index("tokens"))
        else:
        ...
        self.buttonGroup_similarityPattern = QButtonGroup_similarityPattern(
            layout=self.verticalLayout_1gram_SimilarityPattern,
    ...
    def extract_1gVocab(self, vocab1gPath=None,  init=None):
        ...
        nlp_selector = kex.NLPSelect(
            columns=columns,
            special_replace=self.config['csvinfo'].get('untracked_token', None)
        )

        self.clean_rawText = nlp_selector.transform(
                self.dataframe_Original
        )  # might not need to set it as self
        # acces the text before it is cleaned
        self.together = nlp_selector.together

        self.tfidf_1g = (
            self.tokenExtractor_1Gram
                .fit_transform(self.clean_rawText)
        )
        self.dataframe_vocab1Gram = kex.generate_vocabulary_df(
            self.tokenExtractor_1Gram, filename=vocab1gPath, init=init)
    ...
    def setAliasFromNgramButton(self, button):
        """
        When check a radio button on the Ngram button group,
        change the alias dependent on the classification
    ...
    def keyPressEvent(self, event):
        ...
        # TODO bad hack to move up and down checkboxes in pyqt!
        if event.key() == Qt.Key_Up:
            if self.tabWidget.currentIndex() == 0:
                list_len = len(self.buttonGroup_similarityPattern.buttons_list)
                enum = None
                for n, button in enumerate(
                ...
                if enum is not None:
                    ...
                    # print(enum, new_idx)
                    (self.buttonGroup_similarityPattern
                     .buttons_list[new_idx]
                     .setFocus()
                     )
    ...
    def closeEvent(self, event):
        ...
        if choice == Qw.QMessageBox.Save:
            print("SAVE AND CLOSE --> vocab 1gram and Ngram,"
            ...
            print("CLOSE NESTOR --> we save your config file"
                  " so it is easier to open it next time")
            if self.database:
                self.database.close()
    ...
    def onClick_saveTrack(self):
        ...
        window_DialogWait = meta.DialogWait(iconPath=self.iconPath)
        rect = self.geometry()
        rect.setHeight(300)
        rect.setWidth(200)
        ...
        tags2_df = kex.tag_extractor(
            ...
            self.clean_rawText_1Gram,
            vocab_df=self.dataframe_vocabNGram[
                self.dataframe_vocabNGram.alias.notna()
            ]
        )
        ...
        self.tag_df = tags_df.join(
            tags2_df.drop(
                axis='columns',
                labels=tags_df.columns.levels[1].tolist(),
                level=1
            )
        )
        ...
        self.label_report_tagCompleteness.setText(
            f'<a href="https://en.wikipedia.org/wiki/'
            f'Positive_and_negative_predictive_values">'
            f'Tag PPV</a>: {tag_pct.mean():.2%} +/- {tag_pct.std():.2%}'
        )
        self.label_report_completeDocs.setText(
            f'Complete Docs: {tag_comp} of {len(self.tag_df)},'
            f' or {tag_comp/len(self.tag_df):.2%}'
        )
        self.label_report_emptyDocs.setText(
            f'Empty Docs: {tag_empt} of {len(self.tag_df)},'
            f' or {tag_empt/len(self.tag_df):.2%}'
        )
        ...
        print("SAVE --> your information has been saved,"
              " you can now extract your result in CSV or HDF5"
              )
        self.setEnabled(True)

    def onClick_saveNewCsv(self):
        """generate a new csv with the original csv and
           the generated token for the document
        :return:
        ...
        if fname != "":
            ...
            # .to_csv(fname)
            print('SAVE --> readable csv with tagged documents saved at: ',
                  str(fname))

    def onClick_saveTagsHDFS(self):
        """generate a new csv with the document and the tag occurences
           (0 if not 1 if )
        :return:
    ...
    def printReportTable(self):
        ...
        tmp_df = pd.concat([
            self.dataframe_vocab1Gram,
            self.dataframe_vocabNGram[self.dataframe_vocabNGram.NE != '']
        ])
        ...
        totaltag = tmp_df[['alias', 'NE']].drop_duplicates(
        ).NE.replace('', 'NA').value_counts()
        print(totaltag)
...
class MyTaggingToolWindow_research(MyTaggingToolWindow):
    ...
    def __init__(
        ...
        saveTime=None,
        savePercentage=None,
        saveNumberToken=None,
        saveNumberUpdate=None
    ):

        # TODO not able to creat more than 1 per computeur because
        # the name is the same this need to change
        ...
        if name == "excavator_data":
            ...
            lacasion = (Path(__file__).parent.parent
                        / 'datasets'
                        / 'excavators-cleaned.csv'
                        )
    ...
    def set_config(
        ...
        **kwargs
        ...
        MyTaggingToolWindow.set_config(self, **kwargs)
    ...
    def saveTime_whenUpdate(self):
        ...
        if howLong >= self.saveTimeInterval[0]:
            ...
            self.dataframe_vocab1Gram.to_csv(
                self.path_saveTime / vocabname, encoding='utf-8-sig')
    ...
    def savePercentage_whenUpdate(self):
        ...
        if self.savePercentage1gram:  # not empty, avoids (?) indexing err #58
            if percent1g >= self.savePercentage1gram[0]:
                ...
                self.dataframe_vocab1Gram.to_csv(
                    self.path_savePercentage / vocabname, encoding='utf-8-sig')
    ...
    def saveNumberToken_whenUpdate(self):
        ...
        if nbToken1g >= self.saveNumberToken1gram[0]:
            ...
            self.dataframe_vocab1Gram.to_csv(
                self.path_saveNumberToken / vocabname, encoding='utf-8-sig')
    ...
    def saveNumberUpdate_whenUpdate(self):
        ...
        if self.numberUpdate1gram_count >= self.saveNumberUpdate1gram[0]:
            ...
            self.dataframe_vocab1Gram.to_csv(
                self.path_saveNumberUpdate / vocabname, encoding='utf-8-sig')
    ...
    def onClick_UpdateToSaveVocab(self):
        ...
        :return:
        """
        # No need to call the super function, because this action
        # is automaticaly executed
    ...
    def stopTime(self):
        ...
        Qw.QMessageBox.question(
            self, 'Pause the app', 'App in pause', Qw.QMessageBox.Ok
        )
...
def openDataframe(path):
    ...
    try:
        ...
        print("WAIT --> your file are not an UTF8 file,"
              " we are searching the good encoding")
        print("(you might want to open it and save it as UTF8 "
              "for the next time, it is way faster))")
...
class QButtonGroup_similarityPattern(Qw.QButtonGroup):
    ...
    def create_checkBoxs(self, token, autoCheck_value=99, checkBox_show=50):
        ...
        for token, score in similar:
            ...
            loc = self.tokenExtractor_1Gram.ranks_[
                self.vocab.index.get_loc(token)]
...
class MyMplCanvas(FigCanvas):
    ...
    def __init__(
        ...
        FigCanvas.setSizePolicy(
            self, Qw.QSizePolicy.Expanding, Qw.QSizePolicy.Expanding)
